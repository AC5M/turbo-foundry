...
contract FooTest is Test {
    ...
    /// @dev Fuzz test that demonstrates counter is always safe and will not overflow
    function testFuzz_counterSafe(uint256 x) external {
        uint256 before = foo.counter();
        foo.echo(x);

        // If we got here, no overflow occurred. Safe to add.
        uint256 after_ = foo.counter();
        assertEq(after_, before + x);
    }
